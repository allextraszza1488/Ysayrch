...
-- -----------------------------------------------------------------------------
-- MONITOR -- pin the panel; without this Hyprland picks the slowest mode it can
-- -----------------------------------------------------------------------------
...
-- host-specific override; host.lua's own hl.monitor() call replaces the pin
-- above by targeting the actual connected output on THIS machine. pcall so a
-- host with no override file (or a syntax error in one) never breaks the rest
-- of this config from loading.
...
local function has_nvidia()
  ...
  local pipe = io.popen("lspci -nn 2>/dev/null | grep -E '\\[0300\\]|\\[0302\\]' | grep -ci '\\[10de:'")
