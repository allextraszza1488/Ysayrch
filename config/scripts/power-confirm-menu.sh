#!/bin/bash
# -----------------------------------------------------------------------------
# The single power-icon button on both bars: Lock/Logout/Restart/Shutdown.
# Lock is non-destructive (nothing closes, just a password overlay) so it
# skips the confirm step; the other three go through confirm-power-action.sh
# for a Yes/No first.
# -----------------------------------------------------------------------------
set -euo pipefail
...
case "$choice" in
  ...
  Logout)   "$HOME/.config/scripts/confirm-power-action.sh" logout ;;
